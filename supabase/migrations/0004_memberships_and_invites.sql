-- 0004_memberships_and_invites.sql (v2, corrected for RLS NEW usage)
-- Creates memberships + invites and updates policies to support parents/caregivers sharing.
...
for update using (
  ...
  or exists (
    select 1 from public.memberships m
    where m.baby_id = id and m.user_id = auth.uid() and m.role = 'parent'
  )
) with check (
  auth.uid() = user_id
  or exists (
...
for select using (
  exists (
    select 1 from public.memberships m
    where m.baby_id = baby_id and m.user_id = auth.uid()
  )
...
for update using (
  exists (
    select 1 from public.memberships m
    where m.baby_id = baby_id and m.user_id = auth.uid() and m.role = 'parent'
  )
  or user_id = auth.uid()
) with check (
  exists (
...
-- Parents/owner can update any values
...
-- Invitee can self-accept their own pending invite (pending -> accepted)
