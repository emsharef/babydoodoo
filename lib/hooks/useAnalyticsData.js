...
export function useAnalyticsData({ babyId, from, to, role }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (error) {
        ...
        let events = data || [];
        if (role === 'viewer') {
          events = events.filter(e => e.event_type !== 'MyMood');
        }
...
function buildAnalytics(events, from, to) {
  ...
  const calendar = buildCalendarData(events, days);
...
function emptyAnalytics() {
  return {
    ...
    calendar: { points: [], days: [], types: new Set() },
...
function buildCalendarData(events, days) {
  const types = new Set();
  const raw = events.map(event => {
    const d = new Date(event.occurred_at);
    const day = dayFromISO(event.occurred_at);
    const timeOfDay = d.getHours() + d.getMinutes() / 60;
    types.add(event.event_type);
    return { day, timeOfDay, eventType: event.event_type, occurredAt: event.occurred_at, meta: event.meta || {} };
  });
  // jitter overlapping points (same day, within ~0.33h / 20min)
  const buckets = new Map();
  raw.forEach(pt => {
    const key = `${pt.day}|${Math.round(pt.timeOfDay * 3)}`;
    if (!buckets.has(key)) buckets.set(key, []);
    buckets.get(key).push(pt);
  });
  buckets.forEach(group => {
    if (group.length <= 1) return;
    const spread = Math.min(0.4, group.length * 0.08);
    group.forEach((pt, i) => {
      pt.timeOfDay += (i - (group.length - 1) / 2) * spread;
      pt.timeOfDay = Math.max(0, Math.min(24, pt.timeOfDay));
    });
  });
  return { points: raw, days, types };
}
